...
class ClinicAppointment extends StatefulWidget {
  final clinicInformation;
  const ClinicAppointment({Key? key, this.clinicInformation})
      : super(key: key);
...
class _ClinicAppointmentState extends State<ClinicAppointment> {
  ...
  Map<String, dynamic> getServiceHours(var weekday) {
    ...
    for (int i = 0; i < doctors.length; i++) {
      if (doctors[i]['name'] == currentDoctor) {
        doctor = doctors[i];
    ...
    if (availableDays[formatWeekday(weekday)]) {
                        ...
                        Expanded(
                          child: Padding(
                            ...
                            child: GridView.builder(
                                ...
                                itemCount:
                                    widget.clinicInformation['services'].length,
  ...
  submitAppointment() async {
    ...
    await addNotification(
        uid: clinicInformation['uid'],
        title: "New Appointment Request",
        body:
            "Hey Doc! A Patient submitted an Appointment Request. Review it Now!");
    await addAppointment(appointmentDetails, "clinic");
                                      ...
                                      child: TextButton(
                                        onPressed: () async {
                                          appointmentDetail['clinicData'] =
                                              await getData('clinics',
                                                  id: appointmentDetail[
                                                      'clinicID']);
                                          appointmentDetail['doctorData'] =
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Padding(
                          padding: const EdgeInsets.only(top: 15),
                          child: Text(
                            "Dr. ${appointmentDetail['doctorData']['name']}",
                            style: textTheme.bodyMedium,
                          ),
                        ),
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Padding(
                                padding: const EdgeInsets.only(top: 15),
                                child: Text(
                                  "${widget.clinicInformation['clinicName']}",
                                  style: textTheme.labelSmall?.copyWith(
                                      fontWeight: FontWeight.w600,
                                      color: appColors['white']),
                                ),
                              ...
                              Padding(
                                ...
                                child: FutureBuilder(
                                  future: placemarkFromCoordinates(
                                      widget.clinicInformation['address']
                                          ['latitude'],
                                      widget.clinicInformation['address']
                                          ['longitude']),
                                  builder: (BuildContext context,
  ...
  void getDoctors() async {
    var clinicID = clinicInformation['clinicID'];
    List doctorsID = await getData("clinics", id: clinicID, field: "doctors");
    var doctors = [];

    for (var doctorID in doctorsID) {
      this.doctors.add(await getData("doctors", id: doctorID));
    }
    log(name: "DOCTORS", doctors.toString());
  }

  void getAvailableDoctors() {
    doctorsName.clear();
    for (var doctor in doctors) {
      if (doctor['specializations'].contains(service)) {
        doctorsName.add(doctor['name']);
        log(name: "Specializations", doctor['specializations'].toString());
      }
    }

    if (doctorsName.isNotEmpty) {
      setState(() {
        currentDoctor = doctorsName.first;
      });
    } else {
      setState(() {
        currentDoctor = "";
  ...
  checkAvailableSlots() async {
    takenSlots = await getTakenSlots(id: clinicInformation['clinicID'], accountType: 'clinic');
  }

  @override
  void initState() {
    clinicInformation = widget.clinicInformation;
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: DropdownButtonFormField(
                                      ...
                                      items: doctorsName.map((String value) {
                            ...
                            service.isNotEmpty && currentDoctor.isNotEmpty
                                ? Column(
                                    crossAxisAlignment:
                                        CrossAxisAlignment.start,
                                    children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            ...
                            if (!(isServiceEmpty || isScheduleInvalid) &&
                                currentDoctor.isNotEmpty) {
                              var duplicate = await checkDuplicateAppointment(
                                  userData["patientID"], pickedTime, "clinic");
